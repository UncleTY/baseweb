        ...
        if (this.hasPage) {
          ...
          fetch.post(this.url, query).then(res => {
            if (res) { //token过期判定
              ...
              if(res[responseType][this.property.total] && res[responseType][this.property.total] > 0 && res[responseType][this.property.rows]) {
                this.tData = res[responseType][this.property.rows]
                this.total = res[responseType][this.property.total]
              } else {
                this.tData = []
                this.total = 0
              }
          ...
          let that = this
          fetch.post(this.url, query).then(res => {
            if (res && res[responseType]) {
              this.loadingData = false
              if (res[responseType][this.property.rows] && res[responseType][this.property.rows].length > 0) {
                this.tData = res[responseType][this.property.rows]
              } else if (res[responseType] && res[responseType].length > 0) {
                this.tData = res[responseType]
              } else if(res[responseType] && res[responseType][this.property.rows] && res[responseType][this.property.rows].length > 0){
                this.tData = res[responseType][this.property.rows]
              } else {
                this.tData = []
              }
